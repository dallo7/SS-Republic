...
app.layout = dbc.Container([
    ...
    dcc.Store(id='active-manual-coord-for-visualizer-highlight', data=None),  # New global store
...
def register_visualizer_callbacks(app_instance, id_suffix=""):
    ...
    def load_callsign_data(n_clicks, callsign_input, num_pages_input):
        if not n_clicks or not callsign_input:
            if ctx.triggered_id is None and n_clicks is None:  # type: ignore
    ...
    @app_instance.callback(
        [Output(f'callsign-flight-map{id_suffix}', 'figure', allow_duplicate=True),
         ...
         Output(f'callsign-path-data-table{id_suffix}', 'style_data_conditional', allow_duplicate=True)],
        Input(f'display-data-store{id_suffix}', 'data'),
        Input('active-manual-coord-for-visualizer-highlight', 'data'),
        State(f'callsign-input-for-map{id_suffix}', 'value')
    )
    def update_displays_from_filtered_data(display_json, active_manual_coords, callsign):
        callsign_title = str(callsign).strip().upper() if callsign else "N/A"

        base_style_conditional = [
            {'if': {'row_index': 'odd'}, 'backgroundColor': '#2C3034'}
        ]
        current_style_conditional = list(base_style_conditional)

        if display_json is None:
            # Assuming create_empty_map_figure_dict_viz and INITIAL_TABLE_COLUMNS_VIZ are available
            empty_fig = create_empty_map_figure_dict_viz(f"No data for {callsign_title}.")
            return empty_fig, [], INITIAL_TABLE_COLUMNS_VIZ, current_style_conditional
        ...
        if active_manual_coords and isinstance(active_manual_coords, dict) and \
                'lat' in active_manual_coords and 'lon' in active_manual_coords and \
                not df.empty and 'LATITUDE' in df.columns and 'LONGITUDE' in df.columns:
            try:
                manual_lat_match = float(active_manual_coords['lat'])  # Already 2 decimal places
                manual_lon_match = float(active_manual_coords['lon'])  # Already 2 decimal places

                for i in range(len(df)):
                    try:
                        row_lat_val = df.loc[i, 'LATITUDE']
                        row_lon_val = df.loc[i, 'LONGITUDE']

                        if pd.notna(row_lat_val) and pd.notna(row_lon_val):
                            row_lat = float(row_lat_val)
                            row_lon = float(row_lon_val)

                            truncated_row_lat = math.trunc(row_lat * 100) / 100.0
                            truncated_row_lon = math.trunc(row_lon * 100) / 100.0

                            if truncated_row_lat == manual_lat_match and \
                                    truncated_row_lon == manual_lon_match:
                                current_style_conditional.append({
                                    'if': {'row_index': i},
                                    'backgroundColor': 'yellow',
                                    'color': 'black'  # Ensures text readability
                                })
                    except (ValueError, TypeError) as e_row_conversion:

                        pass
            except (ValueError, TypeError) as e_manual_conversion:

                pass

        return fig_dict, tbl_data, tbl_cols, current_style_conditional
...
def update_realtime_data_store_inv(n_intervals):
    ...
    try:
        ...
        if 'LAST_UPDATE_TIME' in df_proc.columns and pd.api.types.is_datetime64_any_dtype(df_proc['LAST_UPDATE_TIME']):
            ...
            df_proc['LAST_UPDATE_TIME'] = None  # type: ignore
        ...
        if alert_is_open:
            alert_children = html.Div([html.P(str(alert_children)), html.Hr(), html.P(final_status_msg)])
        else:
    ...
    try:
        ...
        try:
            ...
            if not df_rt.empty:
                ...
                latest_idx = df_rt.loc[
                    df_rt.groupby([FLIGHT_ID_COLUMN, 'DATE'])["LAST_UPDATE_TIME"].idxmax()].index;  # type: ignore
            ...
            if not df_rt.empty:
                latest_idx = df_rt.loc[
                    df_rt.groupby(FLIGHT_ID_COLUMN)["LAST_UPDATE_TIME"].idxmax()].index;  # type: ignore
                df_latest_daily = df_rt.loc[latest_idx].copy()
            else:
                df_latest_daily = pd.DataFrame()
    ...
    try:
        ...
        for col in jub_cols_ordered:
            if col not in df_billing.columns:
                if col in ['LATITUDE', 'LONGITUDE', 'ALTITUDE']:
                    ...
                    df_billing[col] = None  # type: ignore
                else:
                    df_billing[col] = 'N/A'
        ...
        for c_id in cols_for_table_data:
            if c_id in df_display.columns:
                ...
                temp_df_for_dict[c_id] = 'N/A'  # type: ignore
...
@app.callback(
    [Output(f'jub-billing-map{INV_SUFFIX}', 'figure'),
     ...
     Output('active-manual-coord-for-visualizer-highlight', 'data')],
    Input(f'processed-billing-data-store{INV_SUFFIX}', 'data'),
...
def update_jub_billing_map_inv(processed_data, n_clicks_add, sel_rows,
                               ...
                               current_manual_mode):
    triggered_id = ctx.triggered_id if ctx.triggered_id else 'NoTrigger'  # type: ignore
    new_pt_info_children = []  # type: ignore
    clear_lat_input = dash.no_update
    clear_lon_input = dash.no_update
    active_manual_coord_output = dash.no_update
    ...
    if triggered_id == f'jub-add-point-button{INV_SUFFIX}' and n_clicks_add is not None and n_clicks_add > 0:  # type: ignore
        if man_lat is not None and man_lon is not None:
            try:
                ...
                if -90 <= lat <= 90 and -180 <= lon <= 180:
                    ...
                    clear_lat_input, clear_lon_input = None, None  # type: ignore

                    new_pt_info_children.append(
                        html.H6("Last Manually Added Point:", className="text-info small"))  # type: ignore
                    new_pt_info_children.append(
                        html.P(f"Lat: {lat:.4f}, Lon: {lon:.4f}", className="small mb-0"))  # type: ignore
                    new_pt_info_children.append(html.P(f"Status: New Entry", className="small mb-0"))  # type: ignore
                    active_manual_coord_output = {'lat': round(lat, 2), 'lon': round(lon, 2)}
                else:
                    ...
                    active_manual_coord_output = None
            except ValueError:
    ...
    if triggered_id == f'jub-clear-auto-data-button{INV_SUFFIX}':
        ...
        # If clearing map should also clear highlight, set: active_manual_coord_output = None
    elif triggered_id == f'jub-show-all-data-button{INV_SUFFIX}':
        new_manual_mode_status = False
        # If showing all data should clear specific manual highlight, set: active_manual_coord_output = None
    elif triggered_id == f'jub-add-point-button{INV_SUFFIX}' and n_clicks_add is not None and n_clicks_add > 0:  # type: ignore
        new_manual_mode_status = True
    ...
    if new_manual_mode_status:
        ...
        if sel_rows and tbl_data_state:
            ...
            if not df_current_table_view.empty and FLIGHT_ID_COLUMN in df_current_table_view.columns:
                selected_flight_ids = {  # type: ignore
                    df_current_table_view.iloc[i][FLIGHT_ID_COLUMN] for i in sel_rows  # type: ignore
                    if i < len(df_current_table_view) and pd.notna(
                        df_current_table_view.iloc[i].get(FLIGHT_ID_COLUMN))  # type: ignore
                }
                if selected_flight_ids and not df_base.empty:
                    df_selected_from_table = df_base[
                        df_base[FLIGHT_ID_COLUMN].isin(selected_flight_ids)].copy()  # type: ignore
    ...
    if plot_df.empty:
        ...
        return (create_investigation_tool_empty_map_figure(map_message, map_style=JUB_BILLING_MAP_STYLE_INV),
                ...
                new_manual_mode_status, active_manual_coord_output)
    ...
    plot_cols_with_defaults = {
        ...
        FLIGHT_ID_COLUMN: lambda: f"unknown_{datetime.now().timestamp()}",  # type: ignore
    ...
    for col, default_val_or_func in plot_cols_with_defaults.items():
        if col not in plot_df.columns:
            plot_df[col] = default_val_or_func() if callable(
                default_val_or_func) else default_val_or_func  # type: ignore
    ...
    fig_billing = px.scatter_mapbox(plot_df,
                                    ...
                                    custom_data=custom_data_list_for_map,  # type: ignore
    ...
    fig_billing.update_traces(marker=dict(sizemin=5))  # type: ignore

    return (fig_billing, updated_manual_pts, new_pt_info_children or no_update,
            clear_lat_input, clear_lon_input, new_manual_mode_status, active_manual_coord_output)
...
def download_html_report(n_clicks_report, fig_viz_data, table_viz_data,
    ...
    if not n_clicks_report:
        if ctx.triggered_id is None:  # type: ignore
            raise dash.exceptions.PreventUpdate("Initial call, no button click.")
    ...
    def fig_to_base64_img_html(fig_data, width_percent=95, height_px=450, title="Map View"):
        ...
        try:
            ...
            if fig.layout.mapbox:  # type: ignore
                fig.layout.mapbox.style = "carto-positron"  # type: ignore
                if fig.layout.mapbox.center:  # type: ignore
                    fig.layout.mapbox.zoom = fig.layout.mapbox.zoom if fig.layout.mapbox.zoom else 5  # type: ignore
                else:
                    fig.layout.mapbox.center = {"lat": SOUTH_SUDAN_CENTER_LAT_VIZ,
                                                "lon": SOUTH_SUDAN_CENTER_LON_VIZ}  # type: ignore
                    fig.layout.mapbox.zoom = 4  # type: ignore
    ...
    elif table_inv_data_full:
        df_inv_full_for_report = pd.DataFrame(table_inv_data_full)
        if not df_inv_full_for_report.empty:
            temp_billing_df = pd.DataFrame()
